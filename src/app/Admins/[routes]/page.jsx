...
const Dashboard = ({ params }) => {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_BASE_URL}/Admins/${routes}`,
          {
            credentials: "include",
          }
        );
  ...
  if (!isAuthenticated) {
    return <Unauthorized />; // Redirect to Unauthorized if not authenticated
  }
  ...
  return (
    <div className="flex h-screen bg-[#ffffff] ">
      ...
      <div className="flex-1 p-6 ">
        ...
        {pathName == "/Admins/Home" && <AdminHomePage />}
        {pathName == "/Admins/Profile" && <AdminProfile />}
        {pathName == "/Admins/Classes" && <AdminProfile />}
