...
const ShowTeacher = () => {
  ...
  const getTeaceher = async () => {
    setLoading(true)
    const response = await axios.get(`${process.env.NEXT_PUBLIC_API_BASE_URL}/getTeachers/${user?.userId}`)
  ...
  const getSubjectName = (subjectId) => {
    const match = subjectDetails?.find(sub => sub._id === subjectId);
    return match ? match.subjectName : "Unknown Subject";
  };
  const handleDeleteTeacher = async (teacherId) => {
    try {
      const response = await axios.delete(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/deleteTeacher/${teacherId}`
      );
  ...
  return (
    <>
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center h-[70vh] gap-4">
          ...
          <h2 className="text-2xl font-semibold text-gray-800">No Teacher Added Yet</h2>
          <p className="text-gray-600">Get started by adding your first teacher</p>

          <Link href="/admin/Teachers/ChooseClass">
            <button
            ...
            >
              ...
              <p className="translate-x-2">Add Teacher</p>
            </button>
          </Link></div>
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead>
                  <tr className="bg-gray-50">
                    <th className="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Teacher Name</th>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {teacher.map((teacher) => (
                    <tr key={teacher._id} className="hover:bg-gray-50 transition duration-150">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className="px-3 py-1 inline-flex text-sm leading-5 font-semibold rounded-full bg-green-100 text-green-800">
                          {getSubjectName(teacher.teachsubject)}
                        </span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        {getClassName(teacher.teacherClass)}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <Link href={`/admin/Subjects/SubjectDetails/${teacher._id}`}>
                        ...
                        <button className="text-red-600 hover:text-red-900 bg-red-50 p-2 rounded-lg transition duration-200" onClick={() => handleDeleteTeacher(teacher._id)}>
                          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
    ...
    //         </svg>
    //       </div>
    //       <p class="translate-x-2">Add Teacher</p>
    //     </button>
    //     </Link>
    // </div>
  )
